...
void solve() {
    ...
    forsn(i,1,n){
       ...
       sumodd[i]=sumodd[i-1]+add;
    }
    vll sumeven(n,0);
    sumeven[0]=0;
    if (n > 1) sumeven[1] = v[1];
    forsn(i,2,n){
        ll add=0;
        if(i%2!=0)add=v[i];
       sumeven[i]=sumeven[i-1]+add;
    }
    set<ll>check;
    forn(i,n){
        ll sub=sumodd[i]-sumeven[i];
        // cout<<"sumodd[i]: "<<sumodd[i]<<endl;
        // cout<<"sumeven[i]: "<<sumeven[i]<<endl;
        // cout<<"sub: "<<sub<<endl;
        // cout<<check.count(sub)<<endl;
        if(sub==0||check.count(sub)){
            cout<<"YES"<<endl;
            return;
        }
        check.insert(sub);
