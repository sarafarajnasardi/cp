...
ll rec(const vll &a, ll i, ll sum, ll cnt, ll ma, ll maxi) {
    if (cnt == 3) {
        if (sum - ma <= ma) return 0;
        if (sum <= maxi) return 0;
        return 1;
    }
    if (i == a.size()) return 0;

    string key = to_string(i) + "," + to_string(sum) + "," + to_string(cnt) + "," + to_string(ma);
    if (dp.count(key)) return dp[key];

    ll take = rec(a, i + 1, sum + a[i], cnt + 1, max(ma, a[i]), maxi);
    ll notake = rec(a, i + 1, sum, cnt, ma, maxi);
...
void solve() {
    ...
    sort(all(a));
    ll g = a[n-1], s = a[n-2], ans = 0;
    for (ll k = 2; k < n; ++k) {
        ll X = max(2*a[k], (k==n-1?s:g)) - a[k];
        for (ll l = 0, r = k-1; l < r; )
            (a[l] + a[r] > X ? (ans += r - l, --r) : ++l);
    }
    cout << ans << ln;
}
