...
void solve() {
    ...
    if((evensum+oddsum-odd.size())%2==0){
        cout<<evensum+oddsum-(odd.size()-1)<<endl;
        return;
    }else{
        cout<<evensum+oddsum-(odd.size())<<endl;
        return;
    }
    ...
    if(even[0]>odd[0]){
       evensum+=(odd[0]-1);
       evensum-=(even[0]);
       ll x=(evensum)%(odd.size()-1);
       ll add=(evensum)/(odd.size()-1);
