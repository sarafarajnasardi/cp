...
ll rec(vll& a, ll i, ll maxi, ll mask, ll freq1, ll freq3) {
    //dbg(mask);
    if (i == a.size()) {
        if ( mask==7 && freq1 == 1 && freq3 == 1)
            return 1;
        else
            return 0;
    }

    auto key = make_tuple(i, maxi, mask, freq1, freq3);
    if (dp.count(key)) return dp[key];

    ll take = 0;
    if (a[i] >= maxi) {
        ll new_mask = mask | (1 << (a[i] - 1));
        ll f1 = freq1 + (a[i] == 1);
        ll f3 = freq3 + (a[i] == 3);
        if (f1 <= 1 && f3 <= 1) {
            take = rec(a, i + 1, a[i], new_mask, f1, f3);
        }
    }

    ll notake = rec(a, i + 1, maxi, mask, freq1, freq3);
    ll ans=(take+notake+MOD)%MOD;
    if(ans<0){
        ans=(ans+MOD)%MOD;
    }
    return dp[key] = ans;
...
void solve() {
    ...
    vll a(n);
    forn(i, n) cin >> a[i];
    cout << (rec(a, 0, 0, 0, 0, 0)+MOD)%MOD << ln;
}
