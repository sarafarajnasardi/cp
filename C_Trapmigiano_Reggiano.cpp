...
bool dfs_find_path(int u, int p, int target) {
    parent[u] = p;
    if (u == target) return true;
    for (int v : adj[u]) {
        if (v != p && dfs_find_path(v, u, target)) return true;
    }
    return false;
}

void dfs_collect(int u, vector<int>& result) {
    visited[u] = true;
    for (int v : adj[u]) {
        if (!visited[v] && !on_path[v]) {
            dfs_collect(v, result);
        }
    }
    result.push_back(u); 
}
