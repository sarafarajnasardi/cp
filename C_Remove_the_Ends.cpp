...
ll rec(vector<ll> &arr, ll strt, ll end, vector<vector<ll>>& dp) {
    ...
    take+=rec(arr,strt+1,end,dp);
    leave+=rec(arr,strt,end-1,dp);
    ll maxi=max(abs(arr[strt]),abs(arr[end]));
    return dp[strt][end] = max({take, leave,maxi});
}
void solve() {
    ll n;
    cin>>n;
