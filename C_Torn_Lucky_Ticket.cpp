...
void solve() {
    ...
    for (auto x : a) {
        ll l = x.length();
        vector<ll> pref(l, 0), suff(l, 0);

        pref[0] = x[0] - '0';
        for (ll i = 1; i < l; i++) pref[i] = pref[i - 1] + x[i] - '0';

        suff[l - 1] = x[l - 1] - '0';
        for (ll i = l - 2; i >= 0; i--) suff[i] = suff[i + 1] + x[i] - '0';
        ...
        forn(i, l) {
            ...
            if(l%2!=0&&i==l/2){
                    ...
                    ll right = suff[i];
                    ll len = (l-i) - (i);
                    //cout<<right<<" "<<left<<" "<<len<<" "<<ln;
                if (len > 0&&right>left){
                        ans += mp[{right - left, len}];
                        cnt+=mp[{right - left, len}];
                }
            }
            if (i < l / 2) {
            ...
            if(i>=l/2){
                ...
                if (i == l - 1) {
                    ...
                    //cout<<right<<" "<<left<<" "<<len<<" "<<ln;
                    if (len > 0&&left>right){
                        ans += mp[{left - right, len}];
                        cnt+= mp[{left - right, len}];
                    }
                }
            }
        }
